...
function refreshBadge(s) {
  ...
  if (left > 0) {
    ...
    chrome.action.setTitle({ title: `${s.coins} കോയിൻ • ${s.tickets} ടിക്കറ്റ്` });
  }
}
...
const handlers = {
  ...
  async buy({ count = 1 }) {
    return mutate((s) => {
      ...
      const cost = n * ECONOMY.TICKET_PRICE;
      ...
      if (s.coins < cost) {
        return { ok: false, reason: 'coins', message: 'കോയിൻ തികയുന്നില്ല. റീൽസ് കണ്ട് നേടടെ.' };
      }
      s.coins -= cost;
      s.tickets += n;
      s.stats.coinsSpent += cost;
      s.stats.ticketsBought += n;
      return { ok: true, tickets: s.tickets, coins: s.coins };
    });
  },
  ...
  async play({ serial }) {
    return mutate((s) => {
      if (s.tickets < 1) {
        return { ok: false, reason: 'tickets', message: 'ടിക്കറ്റ് ഇല്ലാതെ എന്ത് നറുക്കെടുപ്പ്?' };
      }
      s.tickets -= 1;
      s.stats.plays += 1;
      ...
      if (won) {
        ...
        if (rank < bestRank) s.stats.bestPrize = prize.id;
      } else {
