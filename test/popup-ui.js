...
(async () => {
  ...
  ok('picking a different lot reprints the number', await (async () => {
    const before = q('[data-serial]').textContent;
    q('[data-lotpick="kutty"]').click();
    await sleep(30);
    const after = q('[data-serial]').textContent;
    q(`[data-lotpick="${ctx.PB.DEFAULT_LOT}"]`).click();
    await sleep(30);
    return after !== before;
  })());
  ...
  ok('three lots offered in the popup', q('[data-lots]').querySelectorAll('[data-lotpick]').length === 3);
  ok('the middle lot is preselected',
     q(`[data-lotpick="${ctx.PB.DEFAULT_LOT}"]`).classList.contains('is-on'));
  ok('buy is disabled while broke', labelled('എടുക്ക്').disabled === true);
  ...
  labelled('എടുക്ക്').click();
  await sleep(40);
  ok('buying is reflected in the balance',
     q('[data-hero-v]').textContent === String(100 - ctx.PB.lot(ctx.PB.DEFAULT_LOT).price));
  ...
  ok('extras rendered', q('[data-extras]').querySelectorAll('.toggle').length === 6);
  ok('dialogue clips are offered', q('[data-extras]').textContent.includes('ഡയലോഗ് ക്ലിപ്പുകൾ'));
  ...
  ok('drop zone rendered', !!q('[data-drop]'));
  ok('drop zone accepts many files at once',
     q('[data-dropfile]').getAttribute('multiple') !== null);
  const clipRows = q('[data-clips]').querySelectorAll('[data-clip]');
  ok('one row per sound slot', clipRows.length === ctx.PBFX.sound.allSlots.length);
  ok('the named dialogue slots are all there',
     ctx.PB.MEMES.every((m) => clipRows.some((r) => r.getAttribute('data-clip') === m.id)));
  ok('each names its film',
     clipRows.find((r) => r.getAttribute('data-clip') === 'dinesha')
             .querySelector('.clip__n').textContent.includes('Narasimham'));
  const BUNDLED = ctx.PBFX.sound.BUNDLED_ASSETS || {};
  const unbundledRows = clipRows.filter((r) => !BUNDLED[r.getAttribute('data-clip')]);
  const bundledRows = clipRows.filter((r) => !!BUNDLED[r.getAttribute('data-clip')]);
  ok('unbundled slots start empty',
     unbundledRows.every((r) => r.querySelector('[data-state]').textContent === '—'));
  ok('bundled slots show built-in label',
     bundledRows.length > 0 && bundledRows.every((r) => r.querySelector('[data-state]').textContent === 'ബിൽറ്റ്-ഇൻ'));
  ok('play and clear are disabled with no user clip on unbundled slot',
     unbundledRows[0].querySelector('[data-play]').disabled === true
     && unbundledRows[0].querySelector('[data-clear]').disabled === true);
  ok('play is enabled for bundled slot',
     bundledRows[0].querySelector('[data-play]').disabled === false);
  ok('clear is disabled for bundled-only slot',
     bundledRows[0].querySelector('[data-clear]').disabled === true);
